import React, { useState, useEffect } from 'react';
import { useSocialLinks } from '../../hooks/useSocialLinks';
import { Eye, EyeOff, Plus, Trash2, Share2, ExternalLink } from 'lucide-react';

export default function AdminSocialLinks() {
  const { socialLinks, loading, addSocialLink, updateSocialLinks, deleteSocialLink } = useSocialLinks();
  const [localLinks, setLocalLinks] = useState([]);
  const [saving, setSaving] = useState(false);
  const [adding, setAdding] = useState(false);
  const [status, setStatus] = useState({ message: '', type: '' });

  useEffect(() => {
    if (socialLinks) setLocalLinks(socialLinks);
  }, [socialLinks]);

  const handleFieldChange = (id, field, value) => {
    setLocalLinks((prev) =>
      prev.map((item) => (item.id === id ? { ...item, [field]: value } : item))
    );
  };

  const handleAdd = async () => {
    setAdding(true);
    await addSocialLink({
      platform: 'YouTube',
      url: 'https://youtube.com/@barebottomspa',
      active: true
    });
    setAdding(false);
  };

  const handleDelete = async (id, platform) => {
    if (window.confirm(`Are you sure you want to remove ${platform}? To hide it without deleting, uncheck the visibility toggle instead.`)) {
      await deleteSocialLink(id);
    }
  };

  const handleSave = async () => {
    setSaving(true);
    setStatus({ message: 'Saving…', type: '' });

    const res = await updateSocialLinks(localLinks);

    if (res.success) {
      setStatus({ message: 'Social links saved ✓', type: 'ok' });
    } else {
      setStatus({ message: 'Save failed — please try again.', type: 'err' });
    }

    setSaving(false);
  };

  return (
    <div className="admin-card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Share2 size={18} color="var(--teal)" />
            <span>Social Media Links</span>
          </h3>
          <p style={{ margin: '4px 0 0', color: 'var(--muted)', fontSize: '.85rem' }}>
            Manage the social media icons and links displayed in the website footer.
          </p>
        </div>
        <button
          type="button"
          className="add-btn"
          onClick={handleAdd}
          disabled={adding || loading}
          style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
        >
          <Plus size={14} />
          {adding ? 'Adding…' : '+ Add social account'}
        </button>
      </div>

      <div style={{ marginTop: '18px' }}>
        {localLinks.map((item) => (
          <div
            key={item.id}
            style={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              gap: '12px',
              padding: '12px 14px',
              border: '1px solid #eef4f3',
              borderRadius: '10px',
              marginBottom: '10px',
              background: item.active ? 'white' : '#fafcfc',
              opacity: item.active ? 1 : 0.82,
              flexWrap: 'wrap'
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: '1 1 200px' }}>
              <input
                type="text"
                value={item.platform}
                onChange={(e) => handleFieldChange(item.id, 'platform', e.target.value)}
                placeholder="Platform (e.g. Facebook)"
                style={{
                  width: '140px',
                  padding: '7px 10px',
                  border: '1px solid #d8e6e5',
                  borderRadius: '6px',
                  fontWeight: 600,
                  fontSize: '0.86rem'
                }}
              />
              <input
                type="url"
                value={item.url}
                onChange={(e) => handleFieldChange(item.id, 'url', e.target.value)}
                placeholder="https://..."
                style={{
                  flex: 1,
                  padding: '7px 10px',
                  border: '1px solid #d8e6e5',
                  borderRadius: '6px',
                  fontSize: '0.84rem'
                }}
              />
              {item.url && item.url !== '#' && (
                <a
                  href={item.url}
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: 'var(--teal)', display: 'flex', alignItems: 'center' }}
                  title="Open link in new tab"
                >
                  <ExternalLink size={14} />
                </a>
              )}
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {/* Show / Hide toggle */}
              <label
                style={{
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  gap: '5px',
                  fontSize: '0.8rem',
                  color: item.active ? '#1c8a53' : '#7a9499',
                  background: item.active ? '#e5f8ee' : '#f0f4f4',
                  padding: '4px 10px',
                  borderRadius: '20px',
                  border: item.active ? '1px solid #c2eed5' : '1px solid #dfeceb'
                }}
              >
                <input
                  type="checkbox"
                  checked={Boolean(item.active)}
                  onChange={(e) => handleFieldChange(item.id, 'active', e.target.checked)}
                />
                {item.active ? (
                  <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}>
                    <Eye size={12} />
                    Visible
                  </span>
                ) : (
                  <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <EyeOff size={12} />
                    Hidden
                  </span>
                )}
              </label>

              <button
                type="button"
                onClick={() => handleDelete(item.id, item.platform)}
                style={{
                  background: 'none',
                  border: 'none',
                  color: '#c0442c',
                  cursor: 'pointer',
                  padding: '4px',
                  display: 'flex',
                  alignItems: 'center'
                }}
                title="Delete social link"
              >
                <Trash2 size={14} />
              </button>
            </div>
          </div>
        ))}
      </div>

      <div className="card-footer" style={{ marginTop: '16px' }}>
        <button className="save-btn" onClick={handleSave} disabled={saving || loading}>
          {saving ? 'Saving…' : 'Save social links'}
        </button>
        {status.message && (
          <span className={`status-msg ${status.type}`}>{status.message}</span>
        )}
      </div>
    </div>
  );
}

